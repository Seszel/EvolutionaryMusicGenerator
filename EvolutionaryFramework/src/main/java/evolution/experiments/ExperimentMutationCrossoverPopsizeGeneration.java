...
public class ExperimentMutationCrossoverPopsizeGeneration {
    ...
    private static final List<Integer> POP_SIZE = List.of(100, 150, 200, 250);
    ...
    private static final List<Double> MUTATION_PROBABILITY = List.of(0.05, 0.1, 0.2, 0.25);
    ...
    private static final List<Integer> NUMBER_OF_GENERATIONS = List.of(50, 100, 150, 200);
                ...
                for (double mutationProbability : MUTATION_PROBABILITY) {
                    for (int popsize : POP_SIZE) {
                        for (int generation : NUMBER_OF_GENERATIONS) {
                            System.out.println("\nCrossover: " + crossoverProbability + ", Mutation: " + mutationProbability + ", PopSize: " + popsize + ", Generation" + generation);
                            runAlgorithm(new ImmutablePair<>(new ImmutablePair<>(crossoverProbability, mutationProbability), new ImmutablePair<>(popsize, generation)));
                        }
                    }
                }
            }
    ...
    public static void runAlgorithm(Pair<Pair<Double, Double>, Pair<Integer, Integer>> parameters) {
            ...
            case "NSGA_II":
                ...
                for (int i = 0; i < NUMBER_OF_ITERATIONS; i++) {
                    algorithms[i] = new NSGA_II(
                            ...
                            MELODY_KEY,
                            WEIGHTS,
                            parameters.getKey().getKey(),
                            CROSSOVER_TYPE,
                            parameters.getKey().getValue(),
                            MUTATION_TYPE,
                            SELECTION_TYPE,
                            MATING_POOL_SELECTION_TYPE,
                            parameters.getValue().getValue(),
                            i,
                            CRITERIA,
                            CRITERIA_RANGES,
                            SAVE_TO_JSON,
